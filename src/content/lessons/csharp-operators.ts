...
export const csharpOperatorsLesson: LessonMeta = {
  ...
  concept: [
    ...
    '+ 号身兼两职：数值相加与字符串拼接（string 与任何对象相加都会先转成字符串）。== 号也有双重身份：对内置值类型（int/double 等）比较内容，对引用类型（class）比较引用地址；string 虽是引用类型，却被重载成按内容比较。注意自定义 struct 默认不能用 == 直接比较（除非重载运算符）——这正是值/引用类型知识点的直接体现。',
